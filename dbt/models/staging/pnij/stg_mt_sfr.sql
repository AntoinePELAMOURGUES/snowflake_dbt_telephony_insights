WITH
-- 1. On charge les MT20 SFR
src_mt20 AS (
    SELECT *, 'MT20' AS source_type
    FROM {{ source('pnij_src', 'RAW_MT20') }}
    WHERE source_filename LIKE '%SFR%'
),

-- 2. On charge les MT24 SFR
src_mt24 AS (
...
renamed AS (
    SELECT
        ...
        -- 2. DATES (Spécifique SFR)
        -- Format : "13/10/2025 - 15:43:57 UTC"
        -- On nettoie le suffixe UTC avant conversion
        TRY_TO_TIMESTAMP(
