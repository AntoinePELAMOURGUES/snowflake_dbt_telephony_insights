...
logger = logging.getLogger("telco_logs")
...
log_file_path = os.path.join(logs_dir, "telco_streamlit.log")
...
st.set_page_config(page_title="TELCO", initial_sidebar_state="collapsed")
    ...
    if uploaded_file_1:
        ...
        try:
            response = requests.post(
                f"http://{hostname_fastapi}:8000/telco_mt20_mt24",
                files=files,
            )
            if response.ok:
